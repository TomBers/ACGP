defmodule AcgpWeb.LiveCardsAgainstHumanity do
  ...
  def handle_info(
        %{event: "winner", payload: %{winner: winner, question_card: question_card}},
        socket
      ) do
  ...
  def handle_event("myguess", %{"code" => code, "value" => value}, socket) do
    if code == "Enter" do
      new_guesses = [
        %{answer: value, user: socket.assigns.my_name} | socket.assigns.current_guesses
      ]
  ...
  def handle_event("winner", %{"user" => user}, socket) do
    ...
    AcgpWeb.Endpoint.broadcast_from(pid, channel_id, "winner", %{
      winner: user,
